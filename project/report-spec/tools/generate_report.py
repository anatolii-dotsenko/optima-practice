...
def build_full_report() -> docx.Document:
    ...
    # -----------------------------------------------------------------------
    b.add_centered_paragraph("ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЮ", bold=True)
    b.add_centered_paragraph("«ФАХОВИЙ ПЕРЕДВИЩИЙ КОЛЕДЖ «ОПТІМА»", bold=True)
    b.add_centered_paragraph("Циклова комісія інформаційних технологій та фізико-математичних дисциплін")
    ...
    b.add_centered_paragraph("на тему: «ВЕБСИСТЕМА ОНЛАЙН-ЗАМОВЛЕНЬ У КАВ'ЯРНІ «OPTIMA COFFEE»»", bold=True)
    ...
    b.add_body_paragraph(
        "Практика проведена відповідно до навчального плану підготовки фахівців фахової передвищої освіти "
        "за спеціальністю «Комп'ютерні науки» та наказу директора ТОВ «Фаховий передвищий коледж «Оптіма» "
        "від 15 серпня 2026 року."
    )
    b.add_body_paragraph(
        "Індивідуальне завдання з практики студента групи КН-44 Доценка Анатолія Віталійовича. "
        "Тема проєктного завдання: «Вебсистема онлайн-замовлень у кав’ярні «Optima Coffee»."
    )
    b.add_body_paragraph(
        "Концепція ПЗ та технічні умови: трирівнева архітектура з ізоляцією бізнес-логіки; Python 3.11+, FastAPI, "
        "Pydantic v2, SQLAlchemy 2.0, Alembic; JWT (HS256, строк дії 60 хв), bcrypt, RBAC «клієнт / бариста-адміністратор»; "
        "PostgreSQL 16 в окремому контейнері з томом pgdata; SPA Vanilla JavaScript (360–1920 px); REST API /api/v1 з "
        "OpenAPI 3.1; розгортання в Podman Compose; тестування pytest/httpx (50 тестів); оформлення за ДСТУ 3008:2015."
    )
    b.add_body_paragraph(
    ...
    b.add_body_paragraph(
        ...
        "JWT з алгоритмом HS256 та терміном дії 60 хвилин. Адміністративні ендпоїнти захищені залежністю `get_current_admin_user`, що відхиляє "
